...
const theme = _get(process, 'env.npm_config_theme', 'default'); // 主题样式，默认 `default` 主题
const aThemePath = []; // 主题包-less 文件列表（用于开发环境）
...
// 加载主题包
const loadThemeFn = function () {
  var entryFiles = glob.sync(resolve('./css-theme/' + theme + '/**/*.less'));
  for (let i = 0, len = entryFiles.length; i < len; i++) {
    if (_includes(entryFiles[i], theme)) {
      aThemePath.push(
        `@import "@css-theme/${theme}${entryFiles[i].split(theme)[1]}"`
      );
    }
  }
  console.info('\n主题包：', aThemePath);
...
const baseConfig = {
  ...
  configureWebpack: {
    ...
    resolve: {
      ...
      alias: {
        '@': resolve('./src'),
        ...alias,
        '@assets': resolve('./css-theme/' + theme), // 使用打包的主题路径别名
        vue$: isDev
  ...
  chainWebpack: config => {
    const copyFile = [
      ...
      {
        from: path.resolve(__dirname, '../css-lib-theme'),
        to: isDev
          ? processConfig.dev.cssLibThemeSubDirectory
          : processConfig.build.cssLibThemeSubDirectory,
        ignore: []
      }
    ];
    if (isDev) {
      copyFile.push({
        from: path.resolve(__dirname, '../css-theme'),
        to: isDev
          ? processConfig.dev.cssLibThemeSubDirectory
          : processConfig.build.cssLibThemeSubDirectory,
        ignore: []
  ...
  css: {
    ...
    loaderOptions: {
      less: {
        ...
        prependData: isDev
          ? `@import "@css-theme/common/index.less";@import "@css-theme/common/base-components/index.less";${aThemePath.join(
            ';'
          )};`
          : '@import "@css-theme/common/index.less";' // '@import "@css-theme/index.less";@import "@css-theme/default/index.less";'
      },
      sass: {
        // 配置 sass 全局变量
        prependData: ''
