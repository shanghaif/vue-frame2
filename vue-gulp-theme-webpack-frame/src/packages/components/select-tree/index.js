...
const BaseSelectTree = {
  ...
  methods: {
    ...
    createSelect() {
      ...
      const vNode = h(
        'el-select',
        {
          ...
          nativeOn: {
            click: event => {
              if (!_has(this.$attrs, 'disabled') || (_has(this.$attrs, 'disabled') && this.$attrs.disabled === false)) {
    ...
    createTree() {
      ...
      return h('base-tree', {
        ...
        attrs: {
          ...
          ..._omit(this.$attrs, ['defaultCheckedKeys', 'checkStrictly', 'showCheckbox'])
        },
        props: _assign(
          {},
          {
    ...
    setSingleNode() {
      const that = this;
      const checkedNode = that.$refs[that.treeUserRef].getTree().getNode(that.selectTreeValue);
      if (!_isNil(checkedNode)) {
        const record = checkedNode;
        that.options = [];
        that.options.push({ [that.displayField]: record.data[that.displayField], [that.valueField]: record.data[that.valueField] });
        ...
        that.change({ [that.displayField]: record.data[that.displayField], [that.valueField]: record.data[that.valueField] });
      }
    },
    /**
