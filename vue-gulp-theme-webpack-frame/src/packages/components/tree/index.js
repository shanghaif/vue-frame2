...
const BaseTree = {
  ...
  methods: {
    ...
    updateNode(node, params = {}) {
      if (!_isNil(params) && _hasIn(node, _keys(params))) {
    ...
    createHandleMenu({ node, data }) {
      ...
      let textNode = h('span', { class: 'el-tree-node__label' }, [node.label]);
      ...
      if (_has(this.$scopedSlots, 'default')) {
        ...
        textNode = h(
          ...
          [
            ...
            h('div', { 'base-node-item_wrap': true }, [
              h('el-input', {
                attrs: {
                  maxlength: '30'
                },
                ...
                on: {
                  ...
                  command: val => {
                    ...
                    if (val === this.handleMenuEnum.edit) {
                      if (_get(item, '$attrs.isClose', true)) {
                        this.$refs[
                          `${node[this.nodeKey]}-tree-el-dropdown`
                        ].hide();
                        document.body.click(); // 用于隐藏 isPopconfirm: true 时的气泡确认框
    ...
    changeEditModel(model = true) {
      if (!_isNil(this.editingNode)) {
        const aNodeList = document.getElementsByClassName(
          `item-${this._uid}-${this.editingNode.data[this.valueField]}`
        )[0].childNodes;
        aNodeList[0].style.display = model ? 'none' : 'inline';
        aNodeList[1].style.display = model ? 'inline' : 'none';
        if (!model) {
          this.editingValue = '';
