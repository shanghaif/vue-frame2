<template>
  ...
  >
    <!-- title & description -->
    <div v-if="position === 'up'" class="el-step__main">
    ...
    <!-- title & description -->
    <div v-if="position === 'down'" class="el-step__main">
      <div class="el-step__title" ref="title" :class="['is-' + currentStatus]">
        <slot name="title">{{ title }}</slot>
      </div>
      <div v-if="isSimple" class="el-step__arrow"></div>
...
export default {
  ...
  data() {
    return {
      ...
      lineStyle: {},
      internalStatus: '',
      position: 'down' // 描述信息在时间轴的上面还是下面 up上面、down下面
    };
  },
  ...
  created() {
    if (this.getPanel().position !== 'down') {
      this.position = this.getPanel().position;
    }
  },
  beforeDestroy() {
