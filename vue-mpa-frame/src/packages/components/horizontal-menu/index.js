...
const BaseHorizontalMenu = {
  ...
  methods: {
    ...
    handleDefaultClick() {
      const aChildrenList = _get(
        this.$refs[`${this._uid}-base-horizontal-menu`],
        'children',
        []
      );
     ...
     */
    getEl() {
      return this.$refs[`${this._uid}-base-horizontal-menu`];
    },
    /**
    ...
    createItem() {
      ...
      if (!_isNil(this.buttonGroup)) {
        console.log(this.buttonGroup, 77777);
  ...
  render(h) {
    return h(
      'div',
      {
        ref: `${this._uid}-base-horizontal-menu`,
        attrs: { id: this.$attrs.id },
        class: { 'base-horizontal-menu': true, [this.ctCls]: this.ctCls }
      },
      this.createItem()
    );
  }
};
export default BaseHorizontalMenu;
